...
/**
  ...
  @param {string} realmName - The name of the realm (not the realmID) where the client roles exist - ex: master
  @param {string} id - The id of the client (not the client-id) whose roles should be found
  @param {string} roleName - Name of the role (not the role-id)
  @returns {Promise} A promise that will resolve with the result
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.clients.roles.del(realmName, id, roleName)
        .then((result) => {
        console.log(result) // [{...}]
      })
    })
 */
function del(client) {
  return function del(realm, id, roleName) {
    return new Promise((resolve, reject) => {
      if (!roleName) {
        return reject(new Error('role is missing'));
      }
      ...
      const req = {
        url: `${client.baseUrl}/admin/realms/${realm}/clients/${id}/roles/${roleName}`,
        auth: {
          bearer: privates.get(client).accessToken,
        },
        body: newRole,
